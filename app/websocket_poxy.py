...
class WebSocket:
    ...
    def __init__(self, endpoint, api_key=None, api_secret=None,
                 subscriptions=None, logging_level=logging.DEBUG,
        ...
        # Setup logger.
        #self.logger = logging.getLogger(__name__)
        self.logger = log.setup_custom_logger('websocket-poxy', filename='log/websockets_poxy.log', console=True, log_level=logging.DEBUG) ##dm21376
        ...
        #logger_websocket_poxy.debug(f'Initializing {self.wsName} WebSocket.')
    ...
    def exit(self):
        ...
        self.logger.error(f'WebSocket {self.wsName} exit().')
        self.ws.close()
        while self.ws.sock:
            continue
        self.exited = True
    ...
    def _connect(self, url):
        ...
        self.logger.debug(f'WebSocket {self.wsName} connect().')
        ...
        self.logger.debug(f'WebSocket {self.wsName} Creando hilo con run_forever.')
        ...
        self.logger.debug(f'WebSocket {self.wsName} Configure daemon.')
        ...
        while retries > 0 and (not self.ws.sock or not self.ws.sock.connected):
            retries -= 1
            self.logger.warning(f'WebSocket {self.wsName} Reintendo conexion: {retries}.')
            time.sleep(1)

        # If connection was not successful, raise error.
        if retries <= 0:
            self.logger.error(f'WebSocket {self.wsName} Se superaron los 10 reinentos de conexion.')
            self.exit()
    ...
    def _on_message(self, message):
        ...
        if auth_message:
            ...
            self.logger.debug('Mensaje auth: ' +  str(msg_json))
            self.logger.debug('Mensaje auth: ' +  str(auth_message))
    ...
    def _on_error(self, error):
        ...
        if self.handle_error:
            self.logger.debug(f'WebSocket {self.wsName} Reconnect.')
            self._reset()
            self._connect(self.endpoint)
    ...
    def _reset(self):
        ...
        """
        self.logger.error(f'WebSocket {self.wsName} reset().')
        self.exited = False
        self.auth = False
        self.data = {}
